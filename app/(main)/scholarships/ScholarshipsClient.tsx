...
export default function ScholarshipsClient() {
  ...
  return (
    <div style={{ width: "100%" }}>
            ...
            {!applyingTo ? (
              <div>
                ...
                <div style={{ marginBottom: "3rem", padding: "2rem", background: "rgba(0, 229, 255, 0.03)", border: "1px solid var(--primary)", position: "relative", overflow: "hidden" }}>
                  ...
                  <AnimatePresence>
                    ...
                    {aiMatch && (
                      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ marginTop: "2rem", display: "grid", gridTemplateColumns: "1fr auto", gap: "2rem", alignItems: "center", background: "var(--bg-surface)", padding: "1.5rem", borderLeft: "4px solid var(--primary)" }}>
                         <div>
              ...
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }}>
                 ...
                 {isSuccess ? (
                   ...
                   <div className="sapphire-card" style={{ maxWidth: "800px", margin: "0 auto", padding: "3.5rem" }}>
                      <div style={{ marginBottom: "3rem" }}>
                         <p style={{ fontSize: "0.6rem", fontWeight: "900", color: "var(--primary)", letterSpacing: "0.1em", marginBottom: "0.5rem" }}>APPLYING FOR: {applyingTo.name.toUpperCase()}</p>
                         <h2 style={{ fontSize: "1.25rem", fontWeight: "900" }}>DOCUMENT CHECK</h2>
                      </div>
                      
                      <form onSubmit={handleApply} style={{ display: "grid", gap: "2.5rem" }}>
                        <div>
                           <label style={{ display: "block", marginBottom: "1rem", fontSize: "0.6rem", fontWeight: "900", color: "var(--text-dim)" }}>YOUR FULL NAME</label>
      ...
      <AnimatePresence>
        {previewDoc && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem", backdropFilter: "blur(10px)" }}
          >
             <motion.div 
               initial={{ scale: 0.95, y: 20 }}
               animate={{ scale: 1, y: 0 }}
               style={{ width: "100%", maxWidth: "1000px", background: "var(--bg-surface)", border: "1px solid var(--border-dim)", display: "grid", gridTemplateColumns: "1fr 300px", height: "80vh", overflow: "hidden" }}
             >
                {/* File View Area */}
                <div style={{ background: "#000", position: "relative", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                   <div style={{ textAlign: "center" }}>
                      <div style={{ width: "400px", height: "560px", background: "white", boxShadow: "0 20px 50px rgba(0,0,0,0.5)", position: "relative", padding: "3rem" }}>
                         <div style={{ width: "100%", height: "100%", border: "2px solid #eee", display: "flex", flexDirection: "column", padding: "2rem", textAlign: "left" }}>
                            ...
                            <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: "1rem" }}>
                               ...
                               <div style={{ width: "90%", height: "10px", background: "#f0f0f0" }} />
                               ...
                               <div style={{ width: "70%", height: "10px", background: "#f0f0f0" }} />
                               <div style={{ width: "85%", height: "10px", background: "#f0f0f0" }} />
                            </div>
                      ...
                      <p style={{ color: "#666", fontSize: "0.6rem", marginTop: "2rem", fontWeight: "800", letterSpacing: "0.2em" }}>PREVIEW MODE • SECURE PORTAL</p>
                   </div>
                   
                   <button 
                     onClick={() => setPreviewDoc(null)}
                     style={{ position: "absolute", top: "2rem", right: "2rem", background: "rgba(255,255,255,0.1)", border: "none", color: "white", padding: "1rem", cursor: "pointer", borderRadius: "50%" }}
                   >
                     <X size={20} />
                   </button>
                </div>
                ...
                <div style={{ padding: "2.5rem", display: "flex", flexDirection: "column", borderLeft: "1px solid var(--border-dim)" }}>
                   ...
                   <div style={{ marginBottom: "3rem" }}>
                      <div style={{ display: "flex", alignItems: "center", gap: "1rem", padding: "1rem", background: "var(--bg-accent)", border: "1px solid var(--border-dim)", marginBottom: "1rem" }}>
                         <Activity size={16} color="var(--primary)" />
                         <div>
                            <p style={{ fontSize: "0.5rem", fontWeight: "900", color: "var(--text-dim)" }}>CURRENT STATUS</p>
                            <p style={{ fontSize: "0.7rem", fontWeight: "900", color: previewData?.status === "Approved" ? "#10b981" : previewData?.status === "Rejected" ? "#ef4444" : "#f59e0b" }}>
                               {previewData?.status?.toUpperCase()}
                            </p>
                         </div>
                      </div>
                      <div style={{ padding: "1rem", background: "var(--bg-accent)", border: "1px solid var(--border-dim)" }}>
                         <p style={{ fontSize: "0.5rem", fontWeight: "900", color: "var(--text-dim)", marginBottom: "0.5rem" }}>SYSTEM REMARKS</p>
                         <p style={{ fontSize: "0.65rem", fontWeight: "700", color: "var(--text-main)", fontStyle: "italic" }}>
                            {previewData?.remarks || "NO REMARKS PROVIDED"}
                         </p>
                      </div>
                   </div>
                   ...
                   <div style={{ marginTop: "auto", display: "grid", gap: "1rem" }}>
                      <button 
                        onClick={async () => {
                           await verifyDocument(previewDoc.userId, previewDoc.name, "Approved", "Document verified and approved by OSAS.");
                           setPreviewDoc(null);
                        }}
                        style={{ width: "100%", padding: "1.25rem", background: "#10b981", color: "white", border: "none", fontSize: "0.65rem", fontWeight: "900", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem" }}
                      >
                         <Check size={18} /> APPROVE DOCUMENT
                      </button>
                      <button 
                        onClick={async () => {
                           const remarks = prompt("Enter rejection reason:");
                           if (remarks) {
                              await verifyDocument(previewDoc.userId, previewDoc.name, "Rejected", remarks);
                              setPreviewDoc(null);
                           }
                        }}
                        style={{ width: "100%", padding: "1.25rem", background: "rgba(239, 68, 68, 0.1)", border: "1px solid #ef4444", color: "#ef4444", fontSize: "0.65rem", fontWeight: "900", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem" }}
                      >
                         <AlertCircle size={18} /> REJECT DOCUMENT
                      </button>
                   </div>
                </div>
             </motion.div>
      ...
      <AnimatePresence>
        {isVerifyingIdentity && (
           <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", backdropFilter: "blur(12px)", zIndex: 2000, display: "flex", alignItems: "center", justifyContent: "center", padding: "2rem" }}>
              <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} style={{ width: "100%", maxWidth: "550px", background: "var(--bg-surface)", border: "1px solid var(--border-dim)", padding: "3.5rem", position: "relative", overflow: "hidden" }}>
                 ...
                 <p style={{ fontSize: "0.75rem", color: "var(--text-dim)", fontWeight: "600", marginBottom: "2.5rem", lineHeight: "1.6" }}>
                    Institutional protocol requires a verified full name (First, Middle, and Last) to proceed with scholarship applications. These details will be embedded into your official records.
                 </p>
